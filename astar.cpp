...
template <class T>
void checkNeighbor(Grid *grid, T &openList, std::vector<Node> &closedList, direction d, Node &current, Location &from, Location &to)
{
...
std::vector<Location> astar(Grid *grid, Location from, Location to)
{
    ...
    auto hash = [](const Node &n) { return std::hash<int>()(n.location.getX()) ^ std::hash<int>()(n.location.getY());};
    MyPriorityQueue<Node, std::vector<Node>, decltype(cmp), decltype(hash)> openList(cmp, hash);
