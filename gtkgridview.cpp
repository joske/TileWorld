...
void GridView::drawTile(const Cairo::RefPtr<Cairo::Context> &cr,
...
void GridView::drawAgent(const Cairo::RefPtr<Cairo::Context> &cr,
                         shared_ptr<GridObject> o, int x, int y) {
  ...
  if (agent->hasTile) {
    ...
    cr->arc(x + MAG / 2, y + MAG / 2, MAG / 2, 0, 2 * M_PI);
    cr->begin_new_sub_path();
    draw_text(cr, x + MAG / 4, y,
              to_string(agent->getTile()->getScore()).c_str());
  }
}
